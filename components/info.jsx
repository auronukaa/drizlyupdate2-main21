...
const Info = ({ data }) => {
  ...
  return (
    <div className="flex flex-col">
                ...
                <Button
                  ...
                  disabled={
                    ...
                    ((data?.attributes?.stock === 1) |
                      !data?.attributes?.sub_categories)
                  }
                  className={cn(
      ...
      <p className="mt-3 mb-2 text-xs text-neutral-800">
        Disponueshmëria e produktit :{" "}
        <span className="font-bold text-xs text-black">
          {!data?.attributes?.stock | (data?.attributes?.stock === 1)
            ? "Jashtë stokut"
            : data?.attributes?.stock}
        </span>
      </p>

      {data?.attributes?.stock >= 2 && (
      ...
      <div className="mt-3 flex flex-col sm:flex-row items-center gap-x-3 gap-y-6">
        <button
          disabled={!data?.attributes?.stock | (data?.attributes?.stock === 1)}
          onClick={onCheckout}
      ...
      <p className="mt-4 rounded-md w-full sm:w-[395px] text-xs sm:text-sm text-center text-neutral-800 gap-x-2 sm:gap-x-4 border border-gray-300 p-2 sm:p-4">
        <div>
